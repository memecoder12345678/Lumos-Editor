...
class SourceControlTab(QWidget):
    ...
    def update_git_status(self):
        ...
        try:
            ...
            for item in self.repo.index.diff("HEAD"):
                ...
                child.setData(0, Qt.UserRole, {"type": "file", "state": "staged", "path": item.a_path})
            ...
            modified_item.setData(0, Qt.UserRole, {"type": "header", "state": "modified"})
            for item in self.repo.index.diff(None):
                child = QTreeWidgetItem(modified_item, [item.a_path, "Modified"])
                child.setData(0, Qt.UserRole, {"type": "file", "state": "modified", "path": item.a_path})
            ...
            untracked_item.setData(0, Qt.UserRole, {"type": "header", "state": "untracked"})
            for file_path in self.repo.untracked_files:
                child = QTreeWidgetItem(untracked_item,[file_path, "Untracked"])
                child.setData(0, Qt.UserRole, {"type": "file", "state": "untracked", "path": file_path})
    ...
    def show_context_menu(self, pos):
        ...
        if data["type"] == "file":
            ...
            if state in["modified", "untracked"]:
                ...
                menu.addAction("Discard Changes", lambda: self.discard_file(file_path, state))
            elif state == "staged":
            ...
            if state in ["modified", "untracked"]:
                ...
                menu.addAction("Discard All", lambda: self.discard_all_in_category(state))
            elif state == "staged":
    ...
    def discard_file(self, file_path, state):
        reply = QMessageBox.question(
            self.main_window, "Discard Changes", 
    ...
    def manage_branches(self):
        ...
        branch, ok = QInputDialog.getItem(
            self.main_window, "Branch Management", "Select a branch to checkout:", 
            options, options.index(current) if current in options else 0, False
        )
        
        if ok and branch:
            if branch == "+ Create New Branch...":
                new_branch, ok2 = QInputDialog.getText(self.main_window, "New Branch", "Enter new branch name:")
    ...
    def commit_changes(self):
        ...
        if staged_count == 0:
            reply = QMessageBox.question(
                self.main_window, "No Staged Changes", 
